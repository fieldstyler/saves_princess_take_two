...
RSpec.describe "Save Princess" do
  it "Checks that we can successfully create a grid" do
    game_board = GameBoard2.new(n: 3, p_location: [2, 2], m_location: [0, 1], grid: [["-m-"], ["---"], ["--p"]])
  ...
  it "Prints directions to princess given a game board" do
    game_board1 = GameBoard2.new(n: 5, p_location: [4, 1], m_location: [3, 4], grid: [["-----"], ["-----"], ["-----"], ["----m"], ["-p---"]])
    game_board2 = GameBoard2.new(n: 3, p_location: [0, 0], m_location: [2, 0], grid: [["p--"], ["---"], ["m--"]])
